...
conflict_report_relpath_for_pr() {
  # Usage: conflict_report_relpath_for_pr <pr_id>
  # Path is relative to repo root so it can be declared as a pipeline artifact.
  local pr_id="$1"

  printf 'conflict-reports/pr-%s.txt\n' "$pr_id"
}

all_merge_conflict_blocks_for_file() {
  # Usage: all_merge_conflict_blocks_for_file <file_path> <max_lines>
  # Extracts all conflict blocks (<<<<<<< ... >>>>>>>) from the working tree file,
  ...
  awk -v max="$max_lines" '
...

    {
      if (index($0, "<<<<<<<") == 1) {
        ...
        block++
        print_sep("")
        print_sep(sprintf("----- conflict block %d (starts at line %d) -----", block, NR))
      }
...
write_party_conflict_report_for_pr() {
  # Usage: write_party_conflict_report_for_pr <pr_id> <base_branch> <party_branch> <source_branch> <merge_ref> <conflict_files>
  # Writes/updates a per-PR text report intended to be uploaded as a CI artifact.
  local pr_id="$1"
  local base_branch="$2"
  local party_branch="$3"
  local source_branch="$4"
  local merge_ref="$5"
  local conflict_files="${6:-}"

  local relpath
  relpath=$(conflict_report_relpath_for_pr "$pr_id")
  ...
  {
    printf '%s\n' '=================================================='
    printf 'Merge conflict report (generated by rebuild_party_branch.sh)\n'
    printf 'PR:           %s\n' "$pr_id"
    ...
    git --no-pager diff --name-only --diff-filter=U 2>/dev/null || true
    printf '%s\n' ''
    ...
    local max_lines_per_file
    max_lines_per_file=${REBUILD_PARTY_BRANCH_CONFLICT_REPORT_MAX_LINES_PER_FILE:-2000}
    ...
    if [[ -n "$conflict_files" ]]; then
      ...
      while IFS= read -r f; do
        ...
        if [[ -f "$f" ]]; then
          printf '%s\n' ''
          printf '%s\n' 'Conflict start lines (grep -n ^<<<<<<<):'
          grep -n '^<<<<<<<' "$f" 2>/dev/null || true

          if grep -q '^<<<<<<<' "$f" 2>/dev/null; then
            printf '%s\n' ''
            printf '%s\n' 'Conflict blocks:'
            all_merge_conflict_blocks_for_file "$f" "$max_lines_per_file" || true
          else
            printf '%s\n' ''
            printf '%s\n' 'No inline conflict markers found in file content.'
          fi
        else
          printf '%s\n' ''
          printf '%s\n' 'File not present in working tree; see index stages above (may be delete/modify conflict).'
        fi

        printf '%s\n' ''
      done <<<"$conflict_files"
    else
      printf '%s\n' 'No conflict file list provided; see CI logs.'
    fi
...
upsert_party_conflict_comment() {
  ...
  local conflict_report_path
  conflict_report_path=$(conflict_report_relpath_for_pr "$pr_id")
  ...
  comment_body=$(
    printf '%s\n' \
      ...
      "- Merge ref: \`$merge_ref\`" \
      "- Time (UTC): \`$now\`" \
      "- Full conflict report (pipeline artifact): \`${conflict_report_path}\`" \
      '' \
      'Conflicting files:' \
      '' \
